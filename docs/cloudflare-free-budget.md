...
每个缓存 miss 产生 1 次 D1 查询，按游戏中、等待开始、准备题目、在大厅、本局结算的顺序读取一批最多 21 个候选；“等待开始”直接由既有 `prepared_question_source` 非空判断，不增加查询。20 个候选用于本页，额外 1 个只用于判断是否还有下一页；如果前 20 个候选经权威活跃时间过滤后不足一页，则不返回下一页游标。缓存 hit 不查询 D1 或 Room DO。游标分页不使用 offset；状态、人数、游戏模式和题目来源排序只重排客户端已经取得的房间，不增加请求。这里有意不为活动时间新建索引，避免每次公开房间聚合更新都维护索引；公开房间闲置 48 小时后仍由现有清理任务删除。
