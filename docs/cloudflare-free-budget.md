...
每个缓存 miss 产生 1 次 D1 查询，一次读取当前 runtime generation 的完整候选集合，并按游戏中、等待开始、准备题目、在大厅、本局结算排序；“等待开始”直接由既有 `prepared_question_source` 非空判断，不增加查询。SQL 先用 `updated_at` 排除 48 小时内完全没有 D1 更新的房间，再提前排除超过 1 小时的大厅和准备阶段房间；剩余游戏中和本局结算候选保留给 Room DO 做权威过滤。缓存 hit 不查询 D1 或 Room DO；状态、人数、游戏模式和题目来源排序只重排已经取得的完整结果，不增加请求。这里有意不为活动时间新建索引，避免每次公开房间聚合更新都维护索引。

所有状态超过 1 小时没有有效活跃后不展示；无论状态为何，D1 `updated_at` 超过 48 小时未变化时都不会进入 Room DO 查询。在大厅、准备题目和等待开始通过 D1 `public_activity_at` 提前过滤；D1 状态为 `PLAYING` 或 `GAME_RESULT` 的其余每个候选各产生 1 次小型只读 Room DO 请求，并发 5 个、单次超时 800ms、不重试，再使用最近权威阶段推进时间做最终过滤。该 DO 响应只含状态、人数、阶段推进时间、当前题号和总题数，不进入 mutation 队列，不设置 Alarm、不 checkpoint、不广播、不构建完整 snapshot，也不写 D1/DO SQLite；单个房间失败时回退到 D1 近似人数和公开活动时间，若回退时间也已超过 1 小时则隐藏。普通玩家加入、退出、重连、身份/队伍变化和持久化 checkpoint 不刷新活动时间。

大厅的 `public_activity_at` 和 `spectator_count` 合并进既有房间行更新；游戏阶段推进时间合并进既有 DO aggregate checkpoint，游戏结束时的观战人数合并进既有最终投影，不新增 SQL 语句、checkpoint 或 changed row。游戏中的加入继续只修改 Room DO 权威状态，目录 presence 从同一个 aggregate 计算成员总数和观战人数，不新增存储读取或写入。按每天 60 局且公开房间保留至 48 小时估算，单次完整目录最多约有 120 个需要权威检查的游戏中或结算候选，连同 1 次 D1 读取仍低于 Workers Free 每次调用 1,000 个内部服务子请求的硬上限；并发固定为 5，也低于每次调用最多 6 个同时等待响应头的连接限制。缓存建立后，同一数据中心后续 60 秒内的请求不再调用 Room DO。50 人依次打开目录时通常只由首个请求产生最多约 120 次只读 DO 调用；若 50 人恰好同时命中尚未建立的冷缓存，Cache API 不承诺严格 single-flight，理论上限为约 6,000 次，但写入完成后的请求立即开始复用。按每天 60 局、每局 50 人各打开一次且请求在每局的一分钟窗口内聚集估算，3,000 次 Worker 不变，约 60 次目录重建最多产生约 7,200 次只读 Room DO 调用；无共享缓存时同一假设最多约 360,000 次。跨数据中心或访问分散超过一分钟会分别重建，必须以上线后的 cache hit 与 presence fan-out 指标复核。

同一 48 小时、每天 60 局的上限下，没有活动索引时单次目录重建可能扫描并返回约 120 个候选；若 3,000 次目录读取在每局的一分钟窗口内聚集，约 60 次重建读取 7,200 行 D1，而非无缓存时最多约 360,000 行。取消分页后只有一个完整目录缓存键，不再因游标拆分重复重建。这是以最多约 1 分钟的目录陈旧换取不新增 D1/DO 写入；加入、容量、身份和游戏行为仍由 Room DO 实时权威校验。若生产缓存命中率偏低，应先检查跨数据中心和访问时间分布，再决定是否增加活动时间索引或目录投影。
...
每日 Cron 的房间清理按每批 50 个候选串行执行，在 8 分钟房间批处理时间预算内持续读取和删除，批间等待 1 秒；候选耗尽、时间截止或某批零删除进展时停止。50 只是单批大小，不再构成每天最多删除 50 个房间的总上限。每批产生 1 次候选读取、最多 1 次关联题库/图片读取和 1 次房间级联删除；所有批次关联资源累计后统一确认引用，不为每批重复执行全局图片引用扫描。按每天 60 局基线通常约两批即可收敛；当前生产积压会在同一次 Cron 中按时间预算逐批消化，实际批次数、D1 rowsRead/rowsWritten 和停止原因必须以上线日志与 D1 Analytics 复核。

房间/题集资源清理确认无引用后，按每批最多 1,000 个 key 调用 R2 批量删除；不再为每张图片各产生一次 Worker 内部 R2 请求。该阶段同样遵守每轮最多 10,000 个对象的安全阀：批次失败或超过安全阀而延后的 key 所属题集本轮继续保留，等待下次幂等重试，其他成功批次可以正常收敛。房间/题集阶段和后续全 bucket 对账分别捕获并记录失败，前一阶段失败不再直接跳过后一阶段。
