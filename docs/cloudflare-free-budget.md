...
每个缓存 miss 产生 1 次 D1 查询，一次读取当前 runtime generation 的完整候选集合，并按游戏中、等待开始、准备题目、在大厅、本局结算排序；“等待开始”直接由既有 `prepared_question_source` 非空判断，不增加查询。大厅和准备阶段在 SQL 中提前排除超过 1 小时的房间，游戏中和本局结算候选保留给 Room DO 做权威过滤。缓存 hit 不查询 D1 或 Room DO；状态、人数、游戏模式和题目来源排序只重排已经取得的完整结果，不增加请求。这里有意不为活动时间新建索引，避免每次公开房间聚合更新都维护索引。

所有状态超过 1 小时没有有效活跃后不展示。在大厅、准备题目和等待开始通过 D1 `public_activity_at` 提前过滤；D1 状态为 `PLAYING` 或 `GAME_RESULT` 的每个候选各产生 1 次小型只读 Room DO 请求，并发 5 个、单次超时 800ms、不重试，再使用最近权威阶段推进时间做最终过滤。该 DO 响应只含状态、人数、阶段推进时间、当前题号和总题数，不进入 mutation 队列，不设置 Alarm、不 checkpoint、不广播、不构建完整 snapshot，也不写 D1/DO SQLite；单个房间失败时回退到 D1 近似人数和公开活动时间，若回退时间也已超过 1 小时则隐藏。普通玩家加入、退出、重连、身份/队伍变化和持久化 checkpoint 不刷新活动时间。
