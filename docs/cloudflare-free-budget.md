...
- 远程 URL 导入允许浏览器直接获取或经 Worker 有界代理最多 20MB 原图；缩放、压缩和格式转换在浏览器完成，写入 R2 的最终对象仍不得超过 10MB。
...
JSONL/图片链接导入由出题人浏览器逐张执行“获取原图、Canvas 压缩、立即上传 R2”，移动端并发固定为 1，桌面最多 2；不调用 Cloudflare Images。源站允许 CORS 时每套 30 题产生 30 次上传和 1 次最终 `createUploadedQuestionSet`，共 31 次 Worker 请求。全部源站都需要 Worker 原图代理时，再增加 30 次有界代理和最多 30 行出题权限 D1 读取，共 61 次 Worker 请求、30 行读取、30 次 R2 Class A 写入。按每天 60 套极端估算为 3,660 次 Worker 请求（3.66%）、1,800 行 D1 读取（0.036%）和每月 54,000 次 R2 Class A 写入（5.4%）；Images 转换为 0。该操作只由出题人触发，不随同房间 50 人在线放大，不产生 DO 请求、Alarm、广播或实时补拉。失败重试只处理失败项；刷新或放弃产生的孤儿 R2 对象继续由 72 小时对账清理。
